...
```{r q01}
#ANOVA will be used, DV is the self reported risk attitude, random effects are participants and senario
```
...
```{r q02}
self<-mutate(self, age_cs=age-mean(age))
```
...
```{r q03}
self<-mutate(self, sex=as.factor(sex))
ggplot(self, 
 aes(x = age, 
 y = riskSR)) +
 geom_point() +
...
```{r q05}
#Generally, risk attitude decreases with age. This trend is more outstanding for females than males.
```
...
summary(mod2)
anova(mod2, mod1)
#adding the effect of sex does not improve the model
```
...
```{r q09}
sum_table<-combi%>%group_by(trialtype, RiskyChoice)%>%summarise(n=n(), percentage=n/14401)
#normal->0.2396361; high risk->0.1936671
```
